...
public class ConsultationController {
	...
	public String All(Model model, @RequestParam(name = "search", defaultValue = "") String search, LocalDate d1,
			LocalDate d2, @RequestParam(name = "page", defaultValue = "0") int page,
			@RequestParam(name = "size", defaultValue = "10") int size) {
		// Page<Consultation> consults =
		// consultationRepository.findAll(PageRequest.of(page, size));
		
		d1 = LocalDate.of(2020, 1, 1);
		d2 = LocalDate.now();
		Page<Consultation> consults = consultationRepository.searchUsingDate(search, d1, d2,
	...
	public String searchDateAvancee(Model model, @RequestParam(name = "search", defaultValue = "") String search,
			@RequestParam(name = "date1") String d1, @RequestParam(name = "date2") String d2,
		...
		if (!search.equals("") && !search.trim().equals("")) {
			...
			consults = consultationRepository.searchUsingDate(mots[i], date1, date2,
					PageRequest.of(pageActual, sizePage));
		} else {
			consults = consultationRepository.searchUsingDate(search, date1, date2,
					PageRequest.of(pageActual, sizePage));
		}
		int[] pages = new int[consults.getTotalPages()];
		model.addAttribute("pages", pages);
